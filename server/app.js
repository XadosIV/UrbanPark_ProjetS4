...
app.post('/api/schedule', (req, res) => {
	...
	if (req.body && (!!req.body.role ^ req.body.user) && req.body.parking && req.body.date_start && req.body.date_end){
		PostSchedule(req.body, (err, data) => {
			if (err){
				if(err.code == Errors.E_CONFLICTING_PARAMETERS){
					...
					res.status(400).json({"code":err.code,"message":"Ce créneaux est superposé à un autre pour un/des utilisateur(s) saisi(s)"});
				}else{ // Not generated by us
					console.error(err);
					res.status(500).json({"code":Errors.E_INTERNAL_ERROR, "message":"Une erreur est survenue"});
				}
		...
		res.status(400).json({"code":"E_MISSING_PARAMETER","message":"Champs obligatoires : role*, user*, parking, date_start, date_end. * : Un seul de ces paramètres est requis, les autres ne doivent pas être définits"});
	}
});
