...
const {GetParkings, PutParkings} = require('./db_access/parking');
...
app.post('/api/schedule', (req, res) => {
	...
	if (req.body && (!!req.body.role ^ req.body.user) && req.body.parking && req.body.date_start && req.body.date_end && (isNaN(req.body.first_spot) == isNaN(req.body.last_spot))){
		PostSchedule(req.body, (err, data) => {
			if (err){
				if(err.code == Errors.E_CONFLICTING_PARAMETERS){
					res.status(400).json({"code":err.code,"message":"Un seul champs peut être définit parmis : role, user. Si l'un des champs suivants est définit, les autres doivent être définits aussi : first_spot, last_spot"});
				}else if(err.code == Errors.E_USER_NOT_FOUND){
					res.status(400).json({"code":err.code,"message":"Cet utilisateur n'existe pas"});
					...
					res.status(400).json({"code":err.code,"message":"Ce créneau est superposé à un autre pour un/des utilisateur(s) saisi(s)"});
				}else if(err.code == Errors.E_SPOTS_IN_DIFFERENT_PARKINGS){
					res.status(400).json({"code":err.code,"message":"Les places sélectionnées ne sont pas dans le même parking"});
				}else if(err.code == Errors.E_SPOTS_IN_DIFFERENT_FLOORS){
					res.status(400).json({"code":err.code,"message":"Les places sélectionnées sont dans le même parking mais pas dans le même étage"});
				}else if(err.code == Errors.E_WRONG_SPOT_ORDER){
					res.status(400).json({"code":err.code,"message":"La place de fin ne peut pas être avant la place de début"});
				}else if(err.code == Errors.E_OVERLAPPING_SPOTS){
					res.status(400).json({"code":err.code,"message":"La sélection est superposée à un autre créneau"});
				}else if(err.code == Errors.E_SPOT_NOT_FOUND){
					res.status(400).json({"code":err.code,"message":"L'un des spots de la sélection n'existe pas"});
				}else{ // Not generated by us
					console.error(err);
					res.status(500).json({"code":Errors.E_INTERNAL_ERROR, "message":"Une erreur est survenue"});
				}
...
app.put('/api/schedules/:id', (req, res) => {
	...
	if (req.body && (req.body.user || req.body.parking || req.body.date_start || req.body.date_end)){
		UpdateSchedule(req.body, (err, data) => {
			if (err){
				if(err.code == Errors.E_SCHEDULE_NOT_FOUND){
					res.status(400).json({"code":err.code,"message":"Le créneau demandé n'existe pas."});
				}else if(err.code == Errors.E_DATETIME_FORMAT_INVALID){
					res.status(400).json({"code":err.code,"message":"Le format de la date est invalide"});
				}else if(err.code == Errors.E_WRONG_DATETIME_ORDER){
					res.status(400).json({"code":err.code,"message":"La date de fin ne peut pas précéder la date de commencement"});
				}else if(err.code == Errors.E_OVERLAPPING_SCHEDULES){
					res.status(400).json({"code":err.code,"message":"Ce créneau est superposé à un autre pour l'utilisateur concerné"});
				}else if(err.code == Errors.E_SPOTS_IN_DIFFERENT_PARKINGS){
					res.status(400).json({"code":err.code,"message":"Les places sélectionnées ne sont pas dans le même parking"});
				}else if(err.code == Errors.E_SPOTS_IN_DIFFERENT_FLOORS){
					res.status(400).json({"code":err.code,"message":"Les places sélectionnées sont dans le même parking mais pas dans le même étage"});
					...
					res.status(400).json({"code":err.code,"message":"L'un des spots de la sélection n'existe pas"});
				}else if(err.code == Errors.E_SPOTS_PARKING_DIFFERENT_SCHEDULE_PARKING){
					res.status(400).json({"code":err.code,"message":"Les places sont dans un parking différent de celui attribué par le créneau."});
				}else{ // Not generated by us
					console.error(err);
					res.status(500).json({"code":Errors.E_INTERNAL_ERROR, "message":"Une erreur est survenue"});
				}
